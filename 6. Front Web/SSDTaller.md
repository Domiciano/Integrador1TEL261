...
### spec-critic: el validador
 
Despues de que `spec-writer` genera la spec, debes pasarsela al siguiente skill antes de entregar cualquier tarea al agente de construccion. El critico la lee y emite un veredicto. Si el veredicto es RECHAZADA, debes volver a `spec-writer` con el feedback incluido. El bucle ocurre exactamente una vez: escritor, critico, y si falla, escritor de nuevo con el feedback. Tras la segunda pasada, la spec se acepta tal como queda.
